...
def dedistortion(inter_corner_shape, images, mat_inter, coff_dis):
...
##   cv2.imshow('dst',dst)
    ...
    for fname in images:
        img = cv2.imread(fname)
        newcameramtx, roi = cv2.getOptimalNewCameraMatrix(mat_inter,coff_dis,(img.shape[1],img.shape[0]),0,(img.shape[1],img.shape[0]))
